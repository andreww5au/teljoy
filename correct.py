"""This module defines the TimeRec and CalcPosition classes, containing attributes and
   ...
   The pyephem module defines subclasses of TimeRec and CalcPosition, but the subclasses
   duplicate almost all of the astrometric code and LST calculation here, using routines in
   the ephem library(wrapping Elwood P Downey's code). The routines in here are the original
   Teljoy astrometric code, ported from Pascal, but after some more testing, most of this code
   could be deleted and the classes merged.
"""
...
class TimeRec(object):
    ...
    def CalcJulDay(self):
        ...
        jd = jd + self.UT.day + (self.UT.hour + (self.UT.minute / 60.0) + (
                    self.UT.second / 3600.0) + self.UT.microsecond / 3.6e9) / 24.0
        jd = jd + 1720994 + B + 0.5
        self.JD = jd
    ...
    def CalcLST(self):
        ...
        if not self.JD:  # If JD hasn't been calculated yet, do it now
            self.CalcJulDay()
        T = ((int(
            self.JD - 0.5) + 0.5) - 2415020) / 36525  # Remove fractional day part of JD and convert to centuries since 1900.0
        LST = (modf(0.276919398 + (100.0021359 * T) + (0.000001075 * T * T))[0]) * 24
        LST += MSOLDY * (
                    self.UT.hour + (self.UT.minute / 60.0) + (self.UT.second / 3600.0) + (self.UT.microsecond / 3.6e9))

        L = 279.69668 + (36000.76892 * T) + (0.0003025 * T * T)  # Sun's mean longitude
        Ld = 270.4342 + (481267.8831 * T) - (0.001133 * T * T)  # Moon's mean longitude
...
class CalcPosition(Position):  # Position class defined in globals.py
    ...
    def __getstate__(self):
        ...
        """
        d = {}
        for n in ['Ra', 'Dec', 'Epoch', 'RaA', 'DecA', 'RaC', 'DecC', 'Alt', 'Azi', 'ObjID', 'TraRA', 'TraDEC',
                  'posviolate', 'DomePos']:
            d[n] = self.__dict__[n]
    ...
    def Precess(self):  # Originally in CORRECT.PAS
        ...
        zeta = (((2304.250 + (1.396 * tauz)) * tau) + (0.302 * tau * tau) + (
                    0.018 * tau * tau * tau)) / 3600  # Convert to degrees}
        ...
        B = (cos(DegToRad(theta)) * cos(DegToRad(ObjDec)) * cos(DegToRad(ObjRa + zeta))) - (
                    sin(DegToRad(theta)) * sin(DegToRad(ObjDec)))
        C = (sin(DegToRad(theta)) * cos(DegToRad(ObjDec)) * cos(DegToRad(ObjRa + zeta))) + (
                    cos(DegToRad(theta)) * sin(DegToRad(ObjDec)))
        t = atan2(A, B)
    ...
    def Refrac(self):  # Originally in CORRECT.PAS
        ...
        R = NewR
        CurlR = R * 17 * (prefs.Press * 30 / 1015.92) / (
                    460 + ((prefs.Temp * 9 / 5) + 32))  # convert Temp and Press to F and "Hg for correction}
    ...
    def Nutation(self, T):  # Originally in CORRECT.PAS
        ...
        M = 358.47583 + (35999.04975 * T) - (0.000150 * T * T) - (0.0000033 * T * T * T)  # Sun's mean anomaly
        Md = 296.1046 + (477198.8491 * T) + (0.009192 * T * T)  # Moon's mean anomaly
        Omega = 259.1833 - (1934.1420 * T) + (0.002078 * T * T)  # Longitude of Moon's ascending node
        L = DegToRad(Reduce(L))  # Reduce to range 0-360, convert to radians
        Ld = DegToRad(Reduce(Ld))  # Reduce to range 0-360, convert to radians
        M = DegToRad(Reduce(M))  # Reduce to range 0-360, convert to radians
        Md = DegToRad(Reduce(Md))  # Reduce to range 0-360, convert to radians
    ...
    def Flex(self):
        ...
        d = self.DecC * pi / 3600 / 180  # Dec in radian
        sind = sin(d)
        cosd = cos(d)
        tand = tan(d)
