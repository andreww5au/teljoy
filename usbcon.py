...
class Driver(controller.Driver):
    ...
    def _initialise_configure(self, _):
        ...
        if SITE == 'NZ':
            ...
            for pin_number in [16, 17, 18, 21,
                               22]:  # Pin numbers for limit inputs, which (unlike paddles) are active HIGH
                configuration.pins[
                    pin_number].invert_input = False  # Normally all inputs to be inverted, see top of this method
        #      configuration.shutdown_0_input = 21    # The 'Power' input triggers a hardware shutdown if it goes active
            ...
            for pin_number in [24, 25, 26, 27, 28, 40, 41, 42, 43, 44, 0, 1, 2,
                               3]:  # [0,1,2,3,4, 16,17,18,19,20, 24,25,26,27]
                configuration.pins[
                    pin_number].invert_input = False  # Normally all inputs to be inverted, see top of this method
