...
"""This module handles the low-level motion control - velocity ramping in each axis and sending 
   velocity pairs to the motor queue for each 50ms time step.
"""
...
class MotorControl():
  ...
  def CalcGuide(self):
    """Telescope guide motion is initiated by code that calculates the difference in each axis
       ...
       This function, called once per tick as the motion control values are calculated, uses those
       parameters to calculate the current AXIS_guide velocity for each tick.
       ...
       Entire function not applicable to new USB controller with internal offset guider handling.
    """
    if abs(self.RA_GOffset) > prefs.GuideRate/20:
      self.RA_guide = math.copysign(prefs.GuideRate/20, self.RA_GOffset)
      self.RA_Guidelog += math.copysign(prefs.GuideRate/20, self.RA_GOffset)
      self.RA_GOffset -= math.copysign(prefs.GuideRate/20, self.RA_GOffset)
    else:        #Use up remaining short guide motion
      self.send_RA += self.RA_GOffset
      self.RA_GuideLog += self.RA_GOffset
      self.RA_GOffset = 0

    if abs(self.DEC_GOffset) > prefs.GuideRate/20:
      self.DEC_guide = math.copysign(prefs.GuideRate/20, self.DEC_GOffset)
      self.DEC_Guidelog += math.copysign(prefs.GuideRate/20, self.DEC_GOffset)
      self.DEC_GOffset -= math.copysign(prefs.GuideRate/20, self.DEC_GOffset)
    else:        #Use up remaining short guide motion
      self.DEC_guide = self.DEC_GOffset
      self.DEC_Guidelog += self.DEC_GOffset
      self.DEC_GOffset = 0
...
#No hardware limits readable from Perth at the moment, so CutFrac is always zero.
          ...
          #MIX VELOCITIES for next pulse - add sidereal and non-sidereal rates, plus motion profile velocities
          if self.Teljump:    #We are currently moving in a profiled (ramp-up/plateau/ramp-down) jump
