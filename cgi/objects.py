...
def ZapPeriods(period=0, type='', curs=None):
    ...
    if type:
        curs.execute(
            """update teljoy.objects set period=""" + `period` + " where type='" + type + "' and ObjID not like 'P%'")
    db.commit()
...
def filtobjects(curs=None,
    ...
    curs.execute(
        """select teljoy.objects.ObjID as ObjID, ObjRA, ObjDec from teljoy.objects left join teljoy.objtemp """ +
