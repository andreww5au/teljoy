...
class Driver(controller.Driver):
  ...
  def initialise(self, state_details):
    ...
    self.log("* Initialising", controller.module_version)
...
#    configuration.mc_b_negative_limit_input = controller.PIN_GPIO_7
  ...
  def _get_exception_completed(self, details):
    self.log("Exception Details: %s" % details)
  ...
  def _get_counters_before_stop_completed(self, counters):
    self.log( \
      "Shutdown after frame %s, (%s, %s) total steps before shutdown ramp." % ( \
      counters.reference_frame_number, \
      counters.a_total_steps, \
      counters.b_total_steps))
  ...
  def _stop_now(self):
    # This will stop the run-loop and return normally. If you want to throw an
    # exception, pass a failure (which is a twisted wrapper for an exception) in
    # and it will be thrown by the controller towards the end of run():
    self.host.stop()
...
if __name__ == "__main__":
  ...
  while running:
    ...
    print "controller.Controller.stop() was called with no exception, restarting."
